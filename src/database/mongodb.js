...
module.exports.transactionItemUser = async function (key1, key2, fieldUpdate1, fieldUpdate2) {
	var bulkOp = userSchema.initializeUnorderedBulkOp();
	bulkOp.find({ discordId: key1 }).updateOne({ discordId: key1 }, fieldUpdate1);
	bulkOp.find({ discordId: key2 }).updateOne({ discordId: key2 }, fieldUpdate2);
